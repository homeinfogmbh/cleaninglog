...
class CleaningUser(DigsigdbModel):
    ...
    def add(
        ...
        try:
            ...
            record = cls(name=name, customer=customer, pin=pin, annotation=annotation)
...
class CleaningDate(DigsigdbModel):
    ...
    def add(
        ...
        record = cls(user=user, deployment=deployment, user_timestamp=user_timestamp)
        record.save()

        for annotation in annotations:
            annotation = CleaningAnnotation(cleaning_date=record, text=annotation)
            annotation.save()
    ...
    def select(cls, *args, cascade: bool = False) -> ModelSelect:
        ...
        return (
            ...
            .join(deployment_address, on=Deployment.address == deployment_address.id)
        )
